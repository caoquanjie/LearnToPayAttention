...
paras = np.load('pretrained_model.npy', encoding='latin1').item()
...
def conv_layer(bottom, name, w_shape, b_shape,init_w,init_b):
    with tf.variable_scope(name):
        filt = get_conv_filter(name, w_shape,init=init_w)

        conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')

        conv_biases = get_bias(name, b_shape,init=init_b)
...
def ConvBNReLU(bottom,w_shape,b_shape,axis,phase,name):
    init_weight = paras[name][0]
    init_bias = paras[name][1]
    with tf.variable_scope(name):
        h = conv_layer(bottom,name=name+'/conv',w_shape=w_shape,b_shape=b_shape,init_w=init_weight,
                       init_b=init_bias)
...
def get_conv_filter(name, shape,init):
    return tf.get_variable(name + '_W', dtype=tf.float32, shape=shape,
                           initializer=tf.constant_initializer(init))


def get_bias(name, shape,init):
    return tf.get_variable(name + '_b', dtype=tf.float32, shape=shape,
                           initializer=tf.constant_initializer(init))
...
#         tf.add_to_collection('ga%d' % size, gas)  # (batch,56*56)
#         tf.add_to_collection('img%d'%size,img)
#     ga = tf.get_collection('ga%d' % size)
#     images = tf.get_collection('img%d' % size)
#     ga = tf.convert_to_tensor(ga) # (batch,256,56*56)
...
def compatibility_func(l,g):
    size = l.shape[1]
    L_vector = tf.reshape(l,[size*size,-1]) #(56*56,256)
    g_vector = tf.expand_dims(g,axis=0)
    g_vectorT = tf.transpose(g_vector,(1,0))
    score_vector = tf.matmul(L_vector,g_vectorT) #(56*56,1)
    score = tf.nn.softmax(score_vector)
    img = tf.reshape(score_vector,[size,size])
...
def batch_process(L,G):
    fn = lambda x: compatibility_func(x[0],x[1])
    elems = (L,G)
    gas,images = tf.map_fn(fn, elems, dtype=(tf.float32, tf.float32))
    ga = tf.reduce_mean(gas,[2])
    print(ga.shape)
    print(images.shape)
    return ga,images
